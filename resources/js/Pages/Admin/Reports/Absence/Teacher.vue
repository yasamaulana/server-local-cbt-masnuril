<template>
    ...
    <Head>
        <title>Laporan Absensi Guru - Aplikasi Absensi Online</title>
    </Head>
                    ...
                    <div class="card-body">
                        ...
                        <form @submit.prevent="filter">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <label class="control-label" for="lesson_id">Mata Pelajaran</label>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <template v-for="(classrooms, teacherName) in attendance" :key="teacherName">
                                        <template v-for="(records, classroom) in classrooms" :key="classroom">
                                            <tr v-for="(teacher, index) in records" :key="index">
                                                <td v-if="index === 0"
                                                    :rowspan="Object.keys(classrooms).reduce((sum, key) => sum + classrooms[key].length, 0)">
                                                    {{ teacher.name }}</td>
                                                <td v-if="index === 0" :rowspan="records.length">{{ teacher.classroom }}
                                                </td>
                                                <td>{{ teacher.lesson }}</td>
                                                <td v-for="(status, day) in teacher.attendance" :key="day">
...
export default {
    ...
    setup(props) {
        const form = reactive({
            ...
            lesson_id: '' || (new URL(document.location)).searchParams.get('lesson_id'),
        });

        const filter = () => {
            Inertia.get('/admin/presence-teacher', {
                month: form.month,
        ...
        const exportUrl = computed(() => {
            ...
            if (form.lesson_id) params.append('lesson_id', form.lesson_id);
            return `/admin/presence-teacher/export?${params.toString()}`;
        });

        const statusClass = (status) => {
            switch (status) {
                case 'A': return 'text-danger';
                case 'H': return 'text-success';
