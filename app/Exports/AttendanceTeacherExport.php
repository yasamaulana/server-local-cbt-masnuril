...
class AttendanceTeacherExport implements FromCollection, WithMapping, WithHeadings, WithStyles
{
    ...
    public function map($row): array
    {
        ...
        $mapped = [$row['name'], $row['lesson'], $row['classroom']];
    ...
    {
        $daysInMonth = Carbon::parse($this->month)->daysInMonth;
        $headings = ['Nama', 'Mata Pelajaran', 'Kelas'];
        for ($day = 1; $day <= $daysInMonth; $headings[] = $day++);
        return [
            ["Laporan Absensi Guru: {$this->classroomTitle}, Bulan: {$this->month}"],
            $headings,
        ];
    }
